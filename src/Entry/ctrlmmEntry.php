...
class ctrlmmEntry extends ActiveRecord {
	...
	public static function getImageFolder() {
		return ilCtrlMainMenuPlugin::PLUGIN_ID . "/images";
	}
	...
	public function getTitle() {
		switch ($this->getTitleType()) {
			case self::TITLE_TYPE_IMAGE:
				$image_file = ILIAS_WEB_DIR . "/" . CLIENT_ID . "/" . self::getImageFolder() . "/" . $this->title;
	...
	public function create() {
		if ($this->getParent() > 0) {
			...
			if (!$entry->isChildAllowed($this->getTypeId())) {
				ilUtil::sendFailure(self::plugin()->translate('wrong_child_type'));
		...
		if (!$this->isPermissionCached()) {
			...
			switch ($this->getPermissionType()) {
				...
				case ctrlmmMenu::PERM_ROLE_EXEPTION:
					...
					$assignedRoles = self::dic()->rbacreview()->assignedRoles(self::dic()->user()->getId());
				...
				case ctrlmmMenu::PERM_REF_READ:
					if (self::dic()->access()->checkAccess('read', '', $this->getPermission())) {
						$this->setCachedPermission(true);
					}
					break;
				case ctrlmmMenu::PERM_REF_WRITE:
					if (self::dic()->access()->checkAccess('write', '', $this->getPermission())) {
						$this->setCachedPermission(true);
					}
					break;
				case ctrlmmMenu::PERM_USERID:
					$state = in_array(self::dic()->user()->getId(), json_decode($this->getPermission()));
					$this->setCachedPermission($state);
