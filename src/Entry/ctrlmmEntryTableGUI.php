		...
		if (!$obj->getPlugin()) {
			...
			if ($obj->getTypeId() != ctrlmmMenu::TYPE_SEPARATOR) {
				$actions->addItem(self::plugin()->translate('common_edit'), 'edit', self::dic()->ctrl()
					->getLinkTarget($this->parent_obj, ilCtrlMainMenuConfigGUI::CMD_EDIT_ENTRY));
			...
			if ($obj->getTypeId() != ctrlmmMenu::TYPE_ADMIN) {
				$actions->addItem(self::plugin()->translate('common_delete'), 'delete', self::dic()->ctrl()
					->getLinkTarget($this->parent_obj, ilCtrlMainMenuConfigGUI::CMD_DELETE_ENTRY));
			}
			if ($obj->getTypeId() == ctrlmmMenu::TYPE_DROPDOWN) {
				$actions->addItem(self::plugin()->translate('edit_childs'), 'edit_childs', self::dic()->ctrl()
					->getLinkTarget($this->parent_obj, ilCtrlMainMenuConfigGUI::CMD_EDIT_CHILDS));
			}
