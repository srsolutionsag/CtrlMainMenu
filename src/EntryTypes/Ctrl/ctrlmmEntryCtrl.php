...
class ctrlmmEntryCtrl extends ctrlmmEntry {
	...
	public function isActive() {
		if (!$this->isActiveStateCached()) {
			...
			foreach (self::dic()->ctrl()->getCallHistory() as $class) {
	...
	protected function getError() {
		if (!$this->checkCtrl()) {
			return self::plugin()->translate("ilctrl_error");
		}
	...
	protected function checkCtrl() {
		...
		try {
			self::dic()->ctrl()->getLinkTargetByClass($gui_classes, $this->getCmd());
		} catch (Exception $e) {
			if (self::DEBUG) {
				throw $e;
			}
	...
	public function getLink() {
		...
		$link = self::dic()->ctrl()->getLinkTargetByClass($gui_classes, $this->getCmd());
